...
const ConfigEngine = {
    ...
    _hardcodedBuilding: { orientation: 0, W: 30, D: 60, offsetX: 0, offsetY: 0, spacing: 0, count: 1, stackSpacing: 0, anchor: 'center', stories: 1, floorHeight: 9, locked: false },
    ...
    // Snapshot of the active site's saved state from __SITE_DEFAULTS__ (set during init).
    // reset() reverts to this instead of hardcoded values, so each site keeps its identity.
    siteDefaults: null,

    state: {
        lat: 0, lng: 0, rotation: 0,
        isSnapping: false, locked: true, unitMode: 'SF',
        mapOpacity: 60,
    ...
    init: function() {
        ...
        if (saved) {
            ...
            this.state.locked         = saved.locked         ?? true;
            ...
            if (saved.existingBuildings) {
                this.state.existingBuildings = saved.existingBuildings;
                this.state.activeExBuilding  = saved.activeExBuilding ?? (saved.existingBuildings.length ? 0 : -1);
            } else if (saved.existingBuilding) {
                this.state.existingBuildings = [{ polygon: saved.existingBuilding.polygon, locked: false }];
                this.state.activeExBuilding  = 0;
            } else {
                this.state.existingBuildings = [];
                this.state.activeExBuilding  = -1;
            }
        } else {
            // Fall back to __SITE_DEFAULTS__ — load ALL saved fields (mirrors the localStorage path)
            ...
            this.state.locked           = sd.locked           ?? true;
            this.state.setbacks         = sd.setbacks         ?? { front: 10, rear: 10, sideL: 0, sideR: 0 };
            ...
            this.state.setbacksApplied  = sd.setbacksApplied  ?? false;
            this.state.freeDrag         = sd.freeDrag         ?? true;
            ...
            // Migration: old single existingBuilding → existingBuildings array
            if (sd.existingBuildings) {
                this.state.existingBuildings = sd.existingBuildings;
                this.state.activeExBuilding  = sd.activeExBuilding ?? (sd.existingBuildings.length ? 0 : -1);
            } else if (sd.existingBuilding) {
                this.state.existingBuildings = [{ polygon: sd.existingBuilding.polygon, locked: false }];
                this.state.activeExBuilding  = 0;
            } else {
                this.state.existingBuildings = [];
                this.state.activeExBuilding  = -1;
        ...
        // Migrate older builds: ensure per-building stories/floorHeight + W/D keys + locked flag
        this.state.buildings.forEach(function(b) {
            ...
            if (!('locked'      in b)) b.locked      = false;
        });

        // Migrate building offsets: lot-aligned Frame B → geographic feet.
        // effectiveRotation() now returns 0, so offsets must be in geographic feet.
        // Legacy saves (without _geoOffsets flag) stored lot-aligned offsets; convert them.
        var _src = saved || sd;
        if (_src && !_src._geoOffsets && this.state.rotation) {
            var mRad = this.state.rotation * Math.PI / 180;
            var mCos = Math.cos(mRad), mSin = Math.sin(mRad);
            this.state.buildings.forEach(function(b) {
                var ox = b.offsetX || 0, oy = b.offsetY || 0;
                if (ox === 0 && oy === 0) return;
                b.offsetX = ox * mCos - oy * mSin;
                b.offsetY = ox * mSin + oy * mCos;
            });
        }

        // Snapshot the loaded state as siteDefaults — reset() reverts to this
        this.siteDefaults = {
            lat:              this.state.lat,
            lng:              this.state.lng,
            rotation:         this.state.rotation,
            locked:           this.state.locked,
            setbacks:         JSON.parse(JSON.stringify(this.state.setbacks)),
            buildings:        JSON.parse(JSON.stringify(this.state.buildings)),
            activeBuilding:   this.state.activeBuilding,
            ...
            setbacksApplied:  this.state.setbacksApplied,
            freeDrag:         this.state.freeDrag,
            snapEdge:         this.state.snapEdge,
            siteNorthDeg:     this.state.siteNorthDeg,
            frontBearing:     this.state.frontBearing,
            vehicles:          JSON.parse(JSON.stringify(this.state.vehicles)),
            activeVehicle:     this.state.activeVehicle,
            existingBuildings: JSON.parse(JSON.stringify(this.state.existingBuildings || [])),
            activeExBuilding:  this.state.activeExBuilding ?? -1
        };
    },
    ...
    reset: function() {
        localStorage.removeItem('site_state');
        // Revert to the site's saved defaults (snapshot taken during init),
        // NOT hardcoded values — so reset always returns to the current site.
        var d = this.siteDefaults || {};
        this.state.lat              = d.lat             ?? this.defaults.lat;
        this.state.lng              = d.lng             ?? this.defaults.lng;
        this.state.rotation         = d.rotation        ?? this.defaults.rotation;
        this.state.locked           = d.locked          ?? true;
        this.state.setbacks         = d.setbacks ? JSON.parse(JSON.stringify(d.setbacks)) : { front: 10, rear: 10, sideL: 0, sideR: 0 };
        this.state.buildings        = d.buildings ? JSON.parse(JSON.stringify(d.buildings)) : [ Object.assign({}, this.defaultBuilding) ];
        this.state.activeBuilding   = d.activeBuilding  ?? 0;
        this.state.commFront        = d.commFront       ?? false;
        this.state.showBldgDims     = d.showBldgDims    ?? false;
        ...
        this.state.snapEdge         = d.snapEdge        ?? true;
        this.state.siteNorthDeg     = d.siteNorthDeg    ?? 0;
        this.state.frontBearing     = d.frontBearing    ?? 270;
        this.state.vehicles         = d.vehicles ? JSON.parse(JSON.stringify(d.vehicles)) : [];
        this.state.activeVehicle    = d.activeVehicle   ?? -1;
        if (d.existingBuildings) {
            this.state.existingBuildings = JSON.parse(JSON.stringify(d.existingBuildings));
            this.state.activeExBuilding  = d.activeExBuilding ?? (d.existingBuildings.length ? 0 : -1);
        } else if (d.existingBuilding) {
            this.state.existingBuildings = [{ polygon: d.existingBuilding.polygon, locked: false }];
            this.state.activeExBuilding  = 0;
        } else {
            this.state.existingBuildings = [];
            this.state.activeExBuilding  = -1;
