    ...
    window.onload = function() {
        ...
        (function() {
            ...
            if (seBtn) {
                if (s.snapEdge) { seBtn.style.background = '#16a34a'; seBtn.style.color = '#fff'; seBtn.style.border = '2px solid #4ade80'; seBtn.textContent = 'Snap ON'; }
                else            { seBtn.style.background = '';        seBtn.style.color = '';     seBtn.style.border = ''; seBtn.textContent = 'Snap Edge'; }
            }
            // Sync frontBearing input if it exists
            var fbInp = document.getElementById('frontBearingInput');
            if (fbInp) {
                fbInp.value = s.frontBearing ?? 270;
                var fbLbl = document.getElementById('frontBearingCardinal');
                if (fbLbl) fbLbl.textContent = _bearingToCardinal(s.frontBearing ?? 270);
            }
        })();
