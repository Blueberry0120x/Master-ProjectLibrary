...
- `mergedDimKeys` → which dim segments are merged into predecessor
- `chainWOffset`, `chainDOffset` → chain dim repositioning
- `propDimOffsets` → per-dim drag offsets for property line dims
- `parcelPolygon` → edited polygon vertices or null
...
2. In `engine-config.js` → `init()`: load from `SD.saved.newField` with a fallback — in BOTH the localStorage path AND the `__SITE_DEFAULTS__` fallback path
3. In `engine-config.js` → `siteDefaults` snapshot + `reset()`: add the field so it survives reset
4. In the relevant engine: use `ConfigEngine.state.newField`
5. In `engine-export.js` → `save()`: sync the UI input to state before _payload() runs. If the field lives in MapEngine (like dim state), sync from MapEngine in the `typeof MapEngine !== 'undefined'` block
6. Verify: change → save → reload → persists. Then: switch site → switch back → persists. Then: reset → reverts to site default
