...
| Draw setback dimension labels | SetbackEngine | `drawSetbacks()` |
| Merge dimension segments | SetbackEngine | `updateBldgDimLabels()` → `drawChain()` |
| Hide/show dim segments | MapEngine | `hiddenDimKeys` Set |
| Reposition chain dims (drag) | MapEngine | `chainWOffset`, `chainDOffset` |
| Reposition property line dims | MapEngine | `_propDimOffsets` |
