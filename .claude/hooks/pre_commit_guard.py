...
unread pings, AND runs completion_gate.py regression check BEFORE the commit
executes. Exit 2 = BLOCK.
...
def main() -> int:
    ...
    # Run completion gate — regression check against last stable tag
    gate_script = repo_root / "tools" / "completion_gate.py"
    if gate_script.exists():
        ...
        try:
            ...
            if not gate.get("ok", True):
                ...
                print(
                    f"COMMIT BLOCKED — completion_gate failed ({len(issues)} issue(s)):\n"
                    + "\n".join(f"  - {i}" for i in issues),
                    file=sys.stderr,
                )
                return 2
        except (json.JSONDecodeError, ValueError):
            pass  # gate script error — allow commit (don't block on gate crash)
